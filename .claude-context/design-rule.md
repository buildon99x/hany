# Design Rule — Staged Planning Workflow

> Claude Code 세션에서 기능 제안·설계를 다룰 때 따르는 단계적 워크플로우 규칙.
> 단일 소스. `.claude/skills/design-stage.md`, `.claude/commands/stage-start.md`, `.claude/commands/stage-end.md`가 모두 이 파일을 참조하며, 절차 상세는 이 파일에만 둔다(중복 금지).
...
- **사전 `[T]` 추정** (advisory §6 footnote 2): Stage 2 진입 시 외부 IO 호출 ≥3 또는 광역 grep ≥4회 예상되면 해당 Phase 에 `[T]` 마커 사전 추정(사후 retrospective 마킹과 별개 — D6). §5.4 위임 임계 OR 4중 트리거.
- **의무 산출물** (Medium+ 피처): 본 §3 하단의 "Stage 1·2 의무 산출물 표" 참조.
...
3. 위 게이트를 통과한 뒤에만 저장 절차(§5.4)로 진입.
...
- **위임 임계 OR 4중** (advisory §6 footnote 2): 다음 중 하나라도 충족되면 메인 에이전트가 직접 처리하지 않고 서브에이전트(Explore · Plan · general-purpose 중 적합한 것)에 위임한다. 결과 검증은 메인이 첫 ~30줄을 `Read` 로 확인(기존 절차).
  1. 단일 호출 결과 **≥ 50K char**.
  2. **영향 파일 ≥ 4개** (Stage 1 §영향 파일 표 행 수 기준).
...
- **Advisory 게이트 6종**:
  ...
  - **C4** §5.4 위임 임계 OR 4중(≥50K char / 영향파일 ≥4 / 외부 IO ≥3 / 사전 `[T]` 추정) 권장.
...
## 9. 연동 파일
- `.claude/skills/design-stage.md` — 자연어 트리거 감지 스킬.
- `.claude/commands/stage-start.md` — 슬래시 커맨드(진입).
- `.claude/commands/stage-end.md` — 슬래시 커맨드(종료/저장).
- `.claude/CLAUDE.md` — 본 규칙으로의 포인터.

> 위 4개 연동 파일은 트리거·인자 전달만 담당한다. 절차 본문은 본 문서에만 둔다.
