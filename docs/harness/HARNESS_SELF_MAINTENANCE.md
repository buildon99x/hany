...
> 본 문서는 **harness 자체를 수정**할 때만 적용된다. 일반 feature 작업은 `docs/harness/HARNESS_OPERATING_PLAYBOOK.md` 와 `.claude/skills/design-stage/SKILL.md` 를 따른다.
>
...
| `.claude/skills/design-stage/SKILL.md` | Stage 0/1/2 워크플로우 전체 |
...
5. **자기 적용 회귀 점검**: design-stage SKILL 또는 기타 SKILL 룰을 추가할 때, 그 룰을 본 PR 자체에 소급 적용했을 때 깨지지 않는지 확인 (grandfathering 명시 의무).
...
### C. `.claude/skills/design-stage/SKILL.md` 편집
...
- [ ] skill 동작이 외부 IO (web fetch / MCP / 광역 grep) 를 유발하면 design-stage SKILL §5.4 위임 단일 임계 (≥50K char, Footnote 5로 OR 4중 축소) 안내.

### E. `.claude/commands/**` 편집

- [ ] 커맨드 파일은 **얇은 라우터** 유지 — 절차 본문은 해당 SKILL.md (예: design-stage SKILL) 에만 둔다 (중복 금지).
- [ ] `$ARGUMENTS` 파싱 절차를 design-stage SKILL §2.3 와 일치시킨다.
- [ ] 신규 슬래시는 `CLAUDE.md` Design Workflow 섹션 + 본 가이드 적용 범위 표에 추가.

### F. `docs/harness/**` 편집

- [ ] Playbook ↔ design-stage SKILL ↔ Apply Guide ↔ README 4중 동기화 — 같은 commit 에서 모순 제거.
...
- 동일 design-stage SKILL 절 **9일 5회 변경 = 안티패턴** (lesson 2026-05-09 참조). 한 PR 묶음으로 통합.
...
- design-stage SKILL §3 의무 산출물 표를 줄이는 방향 (게이트 약화).
