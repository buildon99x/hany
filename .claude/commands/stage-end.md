...
2. `.claude-context/design-rule.md` 를 `Read` 로 로드해 §5(저장 규칙) 전체를 따른다.
...
5. **덮어쓰기 가드(§5.3)** — 대상 경로 `docs/feat_{feat-name}_s{stage_number}.md` 가 이미 존재하면 덮어쓰기/다른 이름/취소를 묻는다.
...
세부 규칙은 모두 `.claude-context/design-rule.md` 단일 소스를 따른다.

## 예시
- `/stage-end 1 fever-time` → `docs/feat_fever-time_s1.md` 저장 + 커밋 + `/compact`
- `/stage-end 2` → feat-name 확인 후 `_s2.md` 저장 (Stage 1 문서 존재 확인 포함)
