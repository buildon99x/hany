...
2. `.claude/skills/design-stage/SKILL.md` 를 `Read` 로 로드해 §3(Stage 정의)·§4.1(이전 Stage 자동 복원)·§4.3(모드 정책) 규칙을 따른다.
...
세부 규칙(Stage별 기대 산출물, 코드 금지, 확인 필요 항목, 모드 전환 트리거 등)은 모두 `.claude/skills/design-stage/SKILL.md` 단일 소스를 따른다.
